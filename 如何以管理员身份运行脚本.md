...
3. **运行 PowerShell 脚本**
   ```cmd
   powershell -ExecutionPolicy Bypass -File .\tools\ps1\network\setup-network.ps1
   ```
   然后启动开发服务器：
   ```cmd
   powershell -ExecutionPolicy Bypass -File .\tools\ps1\dev\start-dev.ps1
   ```

## 方法 4: 直接创建快捷方式（最简单）

### 步骤：
1. **在项目目录中，右键点击 `一键修复网络访问.bat`**
   - 选择 "创建快捷方式"
